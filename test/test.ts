...
describe('ncic-lookup', () => {
  ...
  describe('getCodeTypeDescription()', () => {
    it('Returns a string for a valid code type', async () => {
      assert.strictEqual(
        typeof (await ncicLookup.getCodeTypeDescription(validCodeType)),
        'string'
      )
    })
    ...
    it('Returns undefined for an invalid code type', async () => {
      assert.ok(
        typeof (await ncicLookup.getCodeTypeDescription(invalidCodeType)),
        'undefined'
      )
    })
  })
  ...
  describe('getFieldValues()', () => {
    it('Returns an object for a valid code type', async () => {
      assert.ok(
        Object.keys(await ncicLookup.getFieldValues(validCodeType)).length > 0
      )
    })

    it('Returns an empty object for an invalid code type', async () => {
      assert.ok(
        Object.keys(await ncicLookup.getFieldValues(invalidCodeType)).length ===
          0
      )
    })
  })
  ...
  describe('isFieldValue()', () => {
    ...
    it('Returns false for an invalid code type - field value', async () => {
      assert.ok(
        !(await ncicLookup.isFieldValue(invalidCodeType, invalidFieldValue))
      )
    })

    it('Returns false for a valid code type - invalid field value', async () => {
      assert.ok(
        !(await ncicLookup.isFieldValue(validCodeType, invalidFieldValue))
      )
    })
  })
  ...
  describe('getFieldValueDescription()', () => {
    it('Returns a string for a valid code type - field value', async () => {
      assert.strictEqual(
        typeof (await ncicLookup.getFieldValueDescription(
          validCodeType,
          validFieldValue
        )),
        'string'
